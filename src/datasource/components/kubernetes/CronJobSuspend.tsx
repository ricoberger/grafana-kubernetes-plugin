...
export function CronJobSuspend({
  datasource,
  resourceId,
  namespace,
  name,
  ...
  const onConfirm = async () => {
    try {
      ...
      appEvents.publish({
        ...
        payload: [`${namespace}/${name} was suspended`],
      });
    } catch (_) {
      const appEvents = getAppEvents();
      appEvents.publish({
        type: AppEvents.alertError.name,
        payload: [`Failed to suspend ${namespace}/${name}`],
      });
    } finally {
      setIsLoading(false);
      onClose();
  ...
  return (
    <ConfirmModal
      ...
      body={`Are you sure you want to suspend ${namespace}/${name}?`}
      description={
