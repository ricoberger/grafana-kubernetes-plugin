...
export function Resume({
  ...
  const onConfirm = async () => {
    try {
      ...
      appEvents.publish({
        ...
        payload: [`${namespace ? `${namespace}/${name}` : name} was resumed`],
      });
    } catch (_) {
      const appEvents = getAppEvents();
      appEvents.publish({
        ...
        payload: [
          `Failed to resume ${namespace ? `${namespace}/${name}` : name}`,
        ],
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <ConfirmModal
      ...
      body={`Are you sure you want to resume ${namespace ? `${namespace}/${name}` : name}?`}
      description={
