...
export function CSRApproveAction(props: Props) {
  ...
  const onConfirm = async () => {
    try {
      ...
      appEvents.publish({
        ...
        payload: [`${props.name} was approved`],
      });
    } catch (_) {
      const appEvents = getAppEvents();
      appEvents.publish({
        type: AppEvents.alertError.name,
        payload: [`Failed to approve ${props.name}`],
      });
    } finally {
      setIsLoading(false);
      props.onClose();
  ...
  return (
    <ConfirmModal
      ...
      body={`Are you sure you want to approve ${props.name}?`}
      description={
