...
export function NodeUncordonAction(props: Props) {
  ...
  const onConfirm = async () => {
    try {
      ...
      appEvents.publish({
        ...
        payload: [`${props.name} was uncordoned`],
      });
    } catch (_) {
      const appEvents = getAppEvents();
      appEvents.publish({
        type: AppEvents.alertError.name,
        payload: [`Failed to uncordon ${props.name}`],
      });
    } finally {
      setIsLoading(false);
      props.onClose();
  ...
  return (
    <ConfirmModal
      isOpen={props.isOpen}
      title="Uncordon node"
      body={`Are you sure you want to uncordon ${props.name}?`}
      description={
